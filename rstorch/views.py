...
from rstorch.serializers import UserSerializer, StoreSerializer, CategorySerializer
...
class StoreProductList(APIView):
    """
    List all categories of a store instance, or add a new category.
    """
    def get_object(self, pk):
        try:
            return Store.objects.get(pk=pk)
    ...
    def get(self, request, pk, format=None):
        store = self.get_object(pk)
        products = CategorySerializer(store.category_set)
        return Response(products.data)

class StoreSearch(APIView):
    """
    List all stores by search string.
    """
